...
public interface ColorPalette extends SelectableService
{
	 ...
	 * 
	 * Implementations should note that this method can be called by multiple threads at the same time; <i>the caller does not take care of
	 * synchronization</i>. The implementation has to take care of potential synchronization issues itself. (Most implementations should only have to
	 * access the fast storage, which is (in most cases) thread-safe, so this shouldn't be too much of a problem.)
	 * 
	 * @param passes
	 * @return
	 */
	...
	/**
	 * If the JFractalizer was started with command line arguments, some of them are passed to the color palette via this method.
	 * 
	 * @param args
