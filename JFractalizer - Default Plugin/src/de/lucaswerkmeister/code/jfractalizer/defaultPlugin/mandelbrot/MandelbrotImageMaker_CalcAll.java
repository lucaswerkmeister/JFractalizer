...
public class MandelbrotImageMaker_CalcAll extends MandelbrotImageMaker
{
	...
	{
		super(width, height, minReal, maxReal, minImag, maxImag, maxPasses, target, targetX, targetY, palette, superSamplingFactor);
	}
		...
		for (int x = 0; x < width; x++)
			for (int y = 0; y < height; y++)
			{
				...
				averageDenominator = 0; // counting the denominator up is needed because due to rounding errors, sometimes the SuperSampling loop runs
										// less
										// than superSamplingFactor*superSamplingFactor times.
