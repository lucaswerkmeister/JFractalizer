...
public class SimplePalette implements ColorPalette
{
	...
	public void makeFastStorage()
	{
		...
		for (short s = 0; s < colorSteps; s++)
			fastColorStorage.add(new Color((int) Math.round(((colorSteps - 1 - s) / lessColorSteps) * startColor.getRed() + (s / lessColorSteps)
					* endColor.getRed()), (int) Math.round(((colorSteps - 1 - s) / lessColorSteps) * startColor.getGreen() + (s / lessColorSteps)
					* endColor.getGreen()), (int) Math.round(((colorSteps - 1 - s) / lessColorSteps) * startColor.getBlue() + (s / lessColorSteps)
					* endColor.getBlue())));
	}
