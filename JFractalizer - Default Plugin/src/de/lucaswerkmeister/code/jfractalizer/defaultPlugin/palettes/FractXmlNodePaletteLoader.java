...
public class FractXmlNodePaletteLoader extends FractXmlPaletteLoader {
	...
	public void endElement(final String uri, final String localName,
		...
		if (qName.equals("node"))
			palette.nodes.add(new ColorNode(currentStartColor, currentEndColor,
					currentLength));
	...
	public void characters(final char[] ch, final int start, final int length)
		...
		if (elementQNames.size() >= 3
				&& elementQNames.get(elementQNames.size() - 3).equals("node")) {
			if (elementQNames.size() >= 2
				...
				&& elementQNames.get(elementQNames.size() - 2).equals("node")
			...
			switch (elementQNames.getLast()) {
			case "red":
				currentColor = new Color(Integer.parseInt(asString),
						currentColor.getGreen(), currentColor.getBlue(),
						currentColor.getAlpha());
			case "green":
				currentColor = new Color(currentColor.getRed(),
						Integer.parseInt(asString), currentColor.getBlue(),
						currentColor.getAlpha());
			case "blue":
				currentColor = new Color(currentColor.getRed(),
						currentColor.getGreen(), Integer.parseInt(asString),
						currentColor.getAlpha());
			case "alpha":
				currentColor = new Color(currentColor.getRed(),
						currentColor.getGreen(), currentColor.getBlue(),
