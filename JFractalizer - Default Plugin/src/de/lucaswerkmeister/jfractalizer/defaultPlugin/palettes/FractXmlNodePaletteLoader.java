...
public class FractXmlNodePaletteLoader extends FractXmlPaletteLoader {
	...
	public void characters(final char[] ch, final int start, final int length) throws SAXException {
		...
		if (elementQNames.size() >= 3 && elementQNames.get(elementQNames.size() - 3).equals("node")) {
		...
		else if (elementQNames.size() >= 2 && elementQNames.get(elementQNames.size() - 2).equals("node")
				&& elementQNames.getLast().equals("length"))
			...
			switch (elementQNames.getLast()) {
				case "red":
					currentColor = new Color(Integer.parseInt(asString), currentColor.getGreen(),
							currentColor.getBlue(), currentColor.getAlpha());
					break;
				case "green":
					currentColor = new Color(currentColor.getRed(), Integer.parseInt(asString), currentColor.getBlue(),
							currentColor.getAlpha());
					break;
				case "blue":
					currentColor = new Color(currentColor.getRed(), currentColor.getGreen(),
							Integer.parseInt(asString), currentColor.getAlpha());
					break;
				case "alpha":
