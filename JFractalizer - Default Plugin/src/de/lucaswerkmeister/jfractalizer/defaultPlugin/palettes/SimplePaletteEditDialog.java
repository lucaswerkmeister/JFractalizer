...
public class SimplePaletteEditDialog extends Dialog implements ActionListener {
	...
	public SimplePaletteEditDialog(final Frame owner, final SimplePalette start) {
		...
		colorSteps = new JSpinner(new SpinnerNumberModel(start.colorSteps, 2,
				null, 1));
	...
	public SimplePalette getPalette() {
		if (userCanceled)
			return original;
		return new SimplePalette(coreColor.getColor(), startColor.getColor(),
				endColor.getColor(), ((Integer) colorSteps.getValue()));
	}
